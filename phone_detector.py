...
CONF_THRESHOLD = 0.35
COOLDOWN = 15  # seconds per camera
...
def detect_phone(frame, student=None, cam_id=0, exam_mode=False):
    ...
    Normal Mode:
        - student REQUIRED
        - USN-based cooldown
        - Evidence + email handled elsewhere

    Exam Mode:
        - student NOT required
        - camera-level cooldown
        - Evidence handled by exam_evidence.py
    """

    now = time.time()

    # -------- COOLDOWN KEY -------- #
    cooldown_key = cam_id if exam_mode else student["usn"]
